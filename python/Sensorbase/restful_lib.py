...
class Connection:
    ...
    def request(self, resource, method = "get", args = None, body = None, filename=None, headers={}):
        ...
        if filename and body:
            ...
            if args:
                if method == "get":
                    #A very very very bad hack to get hackystat to work.
                    #Can be removed when hackystat parameters work as they
                    #should
                    if args.has_key('startTime') and args.has_key('endTime'):
                        path += u"?" + 'startTime=' + args['startTime'] + \
        ...
        #Hackystat sometimes gives full http path (result)... so if path
        #starts with http: fetch  it as a full path else just fetch the
        #request path with scheme and host set up through initiation
        if self.path.startswith('http:'):
            resp, content = self.h.request(u"%s" % self.path, method.upper(), body=body, headers=headers )
